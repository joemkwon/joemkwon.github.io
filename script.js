// Smooth scrolling comes from `scroll-behavior: smooth` in styles.css; native
// anchor navigation keeps the URL hash and back button working.

// Respect the user's motion preference (live-updates if they change it)
...
bioToggles.forEach(toggle => {
    toggle.addEventListener('click', () => {
        ...
        bioToggles.forEach(t => {
            const active = t === toggle;
...
// Work view toggle: one list; "recent" hides everything after the third entry
const workToggles = document.querySelectorAll('.work-toggle');
const projectList = document.querySelector('.project-list');

workToggles.forEach(toggle => {
    toggle.addEventListener('click', () => {
        workToggles.forEach(t => {
            const active = t === toggle;
            t.classList.toggle('active', active);
            t.setAttribute('aria-pressed', active);
        });
        projectList.classList.toggle('recent-mode', toggle.dataset.view === 'recent');
    });
});

// Per-paper summary expand/collapse (the whole title row toggles)
document.querySelectorAll('.project-toggle').forEach(btn => {
    btn.addEventListener('click', () => {
        const open = btn.closest('li').classList.toggle('open');
...
document.addEventListener('mousemove', (e) => {
    ...
    // Start the follow loop lazily so touch devices never pay for it
    if (!glowLoopStarted) {
        glowLoopStarted = true;
        glowX = mouseX;
        glowY = mouseY;
        animateGlow();
    }
...
function animateGlow() {
    // Stop (and allow a later restart) if the motion preference flips mid-session
    if (reducedMotion) {
        glowLoopStarted = false;
        return;
    }
    const speed = 0.08;
...
around without bunching up or thinning out. Small and pretty and you can
...
useless after about two weeks no matter how good our models get.`,

    voronoi: `Each point owns all the space closer to it than to any other point,
which forms these cells. Cracked mud, packed cells, and the cosmic web
of galaxies all get modeled this way.`,
...
function curlNoise(x, y, t) {
    ...
    const n1 = noise.fbm(x * scale, y * scale + eps + t, 3);
...
function animateVoronoi() {
    ...
    // Draw the actual Voronoi cell walls: each wall connects the circumcenters
    // of two Delaunay triangles that share an edge (the diagram's dual)
    ctx.strokeStyle = getColor(0.55);
    ctx.lineWidth = 0.7;

    const pointIds = new Map();
    voronoiPoints.forEach((p, i) => pointIds.set(p, i));
    const centers = triangles.map(tri => circumcenter(tri.a, tri.b, tri.c));

    const edgeOwner = new Map();
    ctx.beginPath();
    triangles.forEach((tri, ti) => {
        const ia = pointIds.get(tri.a);
        const ib = pointIds.get(tri.b);
        const ic = pointIds.get(tri.c);
        for (const [u, v] of [[ia, ib], [ib, ic], [ic, ia]]) {
            const key = u < v ? u + '-' + v : v + '-' + u;
            if (edgeOwner.has(key)) {
                const c1 = centers[edgeOwner.get(key)];
                const c2 = centers[ti];
                if (c1 && c2) {
...
function initFractal() {
    ...
    // Half resolution for less grain, capped so huge windows stay affordable
    const bufferWidth = Math.min(720, Math.floor(width / 2));
    fractalCanvas.width = bufferWidth;
    fractalCanvas.height = Math.max(1, Math.round(bufferWidth * height / width));
...
function animateFractal() {
    // The c-value drifts only ~1e-4 per frame, so recompute every other frame
    // and reuse the buffer in between, which halves the CPU cost invisibly
    fractalTick++;
    if (fractalTick % 2 === 0) {
    ...
    fractalTime += 0.012; // Faster morphing (2x per computed frame: see throttle above)
...
const modes = {
    ...
    off: { init: clearCanvas, animate: () => {} }
};

// Render one finished-looking frame without running the loop, used when the
// visitor prefers reduced motion
function renderStaticFrame(mode) {
    if (!modes[mode] || mode === 'off') return;
    if (mode === 'lorenz') {
        // Fill the trails first, then draw the whole curve once
        for (let i = 0; i < 1400; i++) lorenzPoints.forEach(t => t.update());
        animateLorenz();
    } else if (mode === 'lissajous') {
        for (let i = 0; i < 1100; i++) lissajousCurves.forEach(c => c.update());
        animateLissajous();
    } else {
        // Trail-based modes need a few steps of warm-up to look composed
        const steps = { ocean: 20, flow: 150, constellation: 2, voronoi: 1, fractal: 1 }[mode] || 1;
...
function resize() {
    ...
    // Reinitialize current mode; restartLoop() handles rendering
    if (modes[currentMode]) {
        modes[currentMode].init();
    }
}

// Fixed-step timing: simulate at 60 steps/sec regardless of display refresh,
// so 120Hz screens don't run everything at double speed
...
function animate(ts) {
    ...
    if (lastFrameTs) {
        frameDebt += Math.min(ts - lastFrameTs, 100);
    }
    lastFrameTs = ts;

    let steps = 0;
    while (frameDebt >= STEP_MS && steps < 2) {
        if (modes[currentMode]) {
    ...
    // Bound the backlog (no spiral after hitches) but keep the sub-step
    // remainder so steady ~30fps displays still reach 60 sim-steps/sec
    if (steps === 2) frameDebt = Math.min(frameDebt, STEP_MS);
}

function restartLoop() {
    cancelAnimationFrame(animationId);
    if (reducedMotion || currentMode === 'off') {
        renderStaticFrame(currentMode);
        return;
    }
    lastFrameTs = 0;
    frameDebt = 0;
...
// Debounced: mobile browsers fire resize continuously while the address bar
// shows/hides, and a full re-init on every event visibly resets the animation
...
// Subtle parallax on hero glow, composed with the glowPulse animation via
// custom properties (a running animation overrides inline transform)
...
if (heroGlow) {
    document.addEventListener('mousemove', (e) => {
        ...
        heroGlow.style.setProperty('--gx', (x * 0.5) + 'px');
        heroGlow.style.setProperty('--gy', (y * 0.5) + 'px');
    });
}
